...
export function ProjectPreview({ name, url, mode = "embed" }: { name: string; url?: string; mode?: "embed" | "external" }) {
  const [viewport, setViewport] = useState<keyof typeof widths>("Desktop");
  if (!url) return null;
  if (mode === "external") return <section className="preview section-rule" aria-labelledby="live-preview"><div className="section-kicker">Live application</div><div className="preview-heading"><div><h2 id="live-preview">See the product live.</h2><p>This application protects itself from embedded previews. Explore the live product directly, then return here for the implementation story and interface details.</p></div></div><div className="external-preview"><p className="external-preview-label">External live site</p><p>{new URL(url).hostname}</p><a className="button button-dark" href={url} target="_blank" rel="noreferrer">Open {name} <ArrowUpRight /></a></div></section>;
